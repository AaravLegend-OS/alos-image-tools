...
.NOTES
    ...
    Exit Codes:
        ...
        Non-zero - Operation failed (typically the exit code from operations).
    ...
    Additional Notes:
        - ExportESD, RecompressESD, ConvertToESD, SaveESD and JoinESD use all your CPU and RAM and can take several hours. (JUST BE PAITENT.)
...
`r`nYes = Save`r`nNo = Discard" -Buttons ([System.Windows.Forms.MessageBoxButtons]::YesNo) -Icon ([System.Windows.Forms.MessageBoxIcon]::Question)
    ...
    'ConvertToWIM' {
        ...
        Set-Progress "ConvertToWIM" "Converting to WIM" 70
        try {
    ...
    'ConvertToESD' {
        Set-Progress "ConvertToESD" "Choosing destination" 10
    ...
    'DeleteImage' {
        ...
        $total = $indices.Count
        if (Question "Do you want to proceed? You cannot reverse this action!" -ne "Yes") { Error "You cancelled the operation." }
        $i = 0
        foreach ($idx in $indices) {
            $i++
            $pct = [int](($i / $total) * 90)
            ...
            try {
                ...
                if ($Missing) {
                    ...
                    if (-not (Test-Path -LiteralPath $ZipPath)) {
                        ...
                        Invoke-WebRequest -Uri "https://github.com/AaravLegend-OS/alos-image-tools/raw/refs/heads/main/ALOS_Image_Tools.zip" -OutFile $ZipPath
                    }
                    $Hash = Get-FileHash -LiteralPath $ZipPath -Algorithm SHA256 | Select-Object -ExpandProperty Hash
                    if ($Hash.ToUpper() -ne "C172C10761998D6B52CDD318621CEC2D60BFD7217B781D62FE14F6B6E12761DE") { Warn "SHA256 hash does not match. ($Hash)"; Clear-Host; return $false } # If the hash does not match, throw an error. An empty hash target always throws an error.
